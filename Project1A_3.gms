$Title Hydroelectric Power Optimization with ability to save water
...
Variable
    ...
    M(i) water left to pass through
    Z total profit;
...
obj.. Z =e= sum((i,t), 3600*P(t)*eta(i)*V(i,t)) + sum((i), (10**6)*M(i)*PF*eta(i));
...
water_left(i).. M(i) =e= Rstart(i) + 3600*(10**(-6))*sum((tt)$(ord(tt) le 9),
                Q(i) - V(i,tt) - S(i,tt) + sum((j)$(deltaBool(j,i) and ord(tt)-delta(j) ge 1),
                V(j,tt-delta(j)) + S(j,tt-delta(j)))) - Rmin(i) + (M('Ahlen')+M('Fjallet'))$(ord(i) = 3) + M('Forsen')$(ord(i)=4)
